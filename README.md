...
CardScope is a local-first, offline-capable trading card scanning application. It uses a Progressive Web App (PWA) frontend and a Python FastAPI backend with OpenCV and Tesseract OCR for card recognition.
...
CardScope is designed to be **local-first**. Your scans and data never leave your device unless you explicitly configure a sync layer (future SaaS expansion).
